...
class ReferenceReader {
            ...
            case 120 /* NamespaceDefinition */:
                {
                    ...
                    this._scopeStackPush(reference_1.Scope.create(lsp.Location.create(this.doc.uri, util.cloneRange(s.location.range))));
    ...
    _methodDeclaration() {
        ...
        let scope = reference_1.Scope.create(lsp.Location.create(this.doc.uri, util.cloneRange(symbol.location.range)));
    ...
    _functionDeclaration() {
        let symbol = this._symbols.shift();
        this._scopeStackPush(reference_1.Scope.create(lsp.Location.create(this.doc.uri, util.cloneRange(symbol.location.range))));
        this._variableTable.pushScope();
        let children = symbol && symbol.children ? symbol.children : [];
        let param;
        for (let n = 0, l = children.length; n < l; ++n) {
