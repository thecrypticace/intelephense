...
function symbolsToTypeReduceFn(prev, current) {
    return typeString_1.TypeString.merge(prev, symbol_1.PhpSymbol.type(current));
}
class ReferenceReader {
    constructor(doc, nameResolver, symbolStore, symbolTable) {
        this.doc = doc;
    ...
    preorder(node, spine) {
        ...
        switch (node.phraseType) {
            ...
            case 26 /* ClassConstElement */:
                this._transformStack.push(new MemberDeclarationTransform(1024 /* ClassConstant */, this._classStack[this._classStack.length - 1].name));
                break;
            case 115 /* MethodDeclarationHeader */:
                this._transformStack.push(new MemberDeclarationTransform(32 /* Method */, this._classStack[this._classStack.length - 1].name));
                break;
            ...
            case 2 /* AnonymousClassDeclaration */:
                {
                    ...
                    this._classStack.push(new typeAggregate_1.TypeAggregate(this.symbolStore, s));
            ...
            case 16 /* ByRefAssignmentExpression */:
                this._transformStack.push(new SimpleAssignmentExpressionTransform(node.phraseType));
                break;
    ...
    _anonymousFunctionCreationExpression() {
        let symbol = this._symbols.shift();
        this._scopeStack.push(symbol);
        let carry = ['$this'];
        let children = symbol && symbol.children ? symbol.children : [];
...
class NamespaceUseClauseTransform {
    constructor(phraseType) {
        this.phraseType = phraseType;
        this.reference = symbol_1.Reference.create(0, '', null);
    }
    push(transform) {
...
class InstanceOfExpressionTransform {
    ...
    push(transform) {
        ...
        else if (transform.phraseType === 100 /* InstanceOfExpression */) {
            this._varType = transform.type;
        }
        switch (transform.phraseType) {
            case 101 /* InstanceofTypeDesignator */:
                this.variable.type = transform.type;
                break;
            case 156 /* SimpleVariable */:
                {
                    let ref = transform.reference;
...
class FunctionCallExpressionTransform {
    ...
    push(transform) {
        switch (transform.phraseType) {
            ...
            case 144 /* RelativeQualifiedName */:
            case 141 /* QualifiedName */:
                this.type = this.referenceSymbolDelegate(transform.reference).reduce(symbolsToTypeReduceFn, '');
                break;
            default:
                break;
        }
...
class SimpleVariableTransform {
    constructor(loc, varTable) {
        ...
        this.reference = symbol_1.Reference.create(256 /* Variable */, '', loc);
    }
    push(transform) {
...
class FullyQualifiedNameTransform {
    constructor(symbolKind, loc) {
        ...
        this.reference = symbol_1.Reference.create(symbolKind, '', loc);
    }
    push(transform) {
...
class MemberNameTransform {
    constructor(loc) {
        ...
        this.reference = symbol_1.Reference.create(0 /* None */, '', loc);
    }
    push(transform) {
        if (transform.tokenType === 83 /* Name */) {
            this.reference.name = transform.text;
...
class HeaderTransform {
    ...
    push(transform) {
        if (transform.tokenType === 83 /* Name */) {
            ...
            this.reference = symbol_1.Reference.create(this._kind, this.nameResolver.resolveRelative(name), loc);
        }
    }
}
class MemberDeclarationTransform {
    ...
    push(transform) {
        if (transform.phraseType === 95 /* Identifier */) {
            let name = transform.text;
            let loc = transform.location;
            this.reference = symbol_1.Reference.create(this._kind, name, loc);
            this.reference.scope = this._scope;
        }
    }
}
class PropertyElementTransform {
    ...
    push(transform) {
        if (transform.tokenType === 84 /* VariableName */) {
            let name = transform.text;
            let loc = transform.location;
            this.reference = symbol_1.Reference.create(16 /* Property */, name, loc);
            this.reference.scope = this._scope;
        }
    }
}
class NamespaceDefinitionTransform {
    push(transform) {
        if (transform.phraseType === 121 /* NamespaceName */) {
            this.reference = symbol_1.Reference.create(512 /* Namespace */, transform.text, transform.location);
        }
    }
}
class ParameterDeclarationTransform {
    push(transform) {
        if (transform.tokenType === 84 /* VariableName */) {
            this.reference = symbol_1.Reference.create(128 /* Parameter */, transform.text, transform.location);
        }
    }
}
class VariableTable {
...
(function (ReferenceReader) {
    function discoverReferences(doc, table, symbolStore) {
        let visitor = new ReferenceReader(doc, new nameResolver_1.NameResolver(), symbolStore, table);
