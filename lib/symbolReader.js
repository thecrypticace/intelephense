...
class SymbolVisitor {
    ...
    functionDeclaration(node, phpDoc) {
        ...
        if (phpDoc) {
            ...
            s.doc = symbol_1.PhpSymbolDoc.create(phpDoc.text, returnTag ? typeString_1.TypeString.nameResolve(returnTag.typeString, this.nameResolver) : '');
        }
        return s;
    }
    ...
    parameterDeclaration(node, phpDoc) {
        ...
        if (phpDoc) {
            ...
            if (tag) {
                s.doc = symbol_1.PhpSymbolDoc.create(tag.description, typeString_1.TypeString.nameResolve(tag.typeString, this.nameResolver));
            }
        }
        if (node.value) {
    ...
    interfaceDeclaration(node, phpDoc, phpDocLoc) {
        ...
        if (phpDoc) {
            s.doc = symbol_1.PhpSymbolDoc.create(phpDoc.text);
    ...
    methodTagToSymbol(tag, phpDocLoc) {
        let s = {
            ...
            modifiers: 256 /* Magic */,
            name: tag.name,
            doc: symbol_1.PhpSymbolDoc.create(tag.description, typeString_1.TypeString.nameResolve(tag.typeString, this.nameResolver)),
            children: [],
            location: phpDocLoc
        };
        if (!tag.parameters) {
    ...
    magicMethodParameterToSymbol(p, phpDocLoc) {
        return {
            ...
            name: p.name,
            modifiers: 256 /* Magic */,
            doc: symbol_1.PhpSymbolDoc.create('', typeString_1.TypeString.nameResolve(p.typeString, this.nameResolver)),
            location: phpDocLoc
        };
    }
    propertyTagToSymbol(t, phpDocLoc) {
        return {
            ...
            doc: symbol_1.PhpSymbolDoc.create('', typeString_1.TypeString.nameResolve(t.typeString, this.nameResolver)),
