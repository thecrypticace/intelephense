...
class NameResolver {
    ...
    qualifiedNamePhraseText(node, kind) {
        if (!node || !node.name) {
            return '';
        }
...
class ExpressionTypeResolver {
    ...
    resolveExpression(node) {
        ...
        switch (node.phraseType) {
            ...
            case 83 /* FullyQualifiedName */:
            case 142 /* RelativeQualifiedName */:
                return new TypeString(this.nameResolver.qualifiedNamePhraseText(node, 1 /* Class */));
    ...
    scopedMemberName(node) {
        ...
        else if (node && parsedDocument_1.ParsedDocument.isPhrase(node.name, [93 /* Identifier */])) {
            return this.nameResolver.tokenText(node.name.name);
        }
        return '';
    }
    classTypeDesignator(node) {
        if (node && parsedDocument_1.ParsedDocument.isPhrase(node.type, [139 /* QualifiedName */ | 83 /* FullyQualifiedName */ | 142 /* RelativeQualifiedName */])) {
            return new TypeString(this.nameResolver.qualifiedNamePhraseText(node.type, 1 /* Class */));
        }
        else if (node && parsedDocument_1.ParsedDocument.isPhrase(node.type, [143 /* RelativeScope */])) {
            return new TypeString(this.nameResolver.thisName);
        }
        else {
            return new TypeString('');
    ...
    functionCallExpression(node) {
        ...
        let functionName = this.nameResolver.qualifiedNamePhraseText(qName, 64 /* Function */);
...
class VariableTypeResolver {
    constructor(variableTable, document, nameResolver, symbolStore, haltAtNode) {
        this.variableTable = variableTable;
        this.document = document;
        this.nameResolver = nameResolver;
        this.symbolStore = symbolStore;
    ...
    preOrder(node, spine) {
        ...
        switch (node.phraseType) {
            ...
            case 153 /* SimpleAssignmentExpression */:
            case 16 /* ByRefAssignmentExpression */:
                if (parsedDocument_1.ParsedDocument.isPhrase(node.left, [154 /* SimpleVariable */, 106 /* ListIntrinsic */])) {
    ...
    _isNonDynamicSimpleVariable(node) {
        return parsedDocument_1.ParsedDocument.isPhrase(node, [154 /* SimpleVariable */]) &&
            parsedDocument_1.ParsedDocument.isToken(node.name, [83 /* Name */]);
    }
    _assignmentExpression(node) {
        let lhs = node.left;
        let rhs = node.right;
        let exprTypeResolver = new ExpressionTypeResolver(this.nameResolver, this.symbolStore, this.variableTable);
        if (parsedDocument_1.ParsedDocument.isPhrase(lhs, [154 /* SimpleVariable */])) {
            let varName = this._simpleVariable(lhs);
            this.variableTable.setType(varName, exprTypeResolver.resolveExpression(rhs));
        }
        else if (parsedDocument_1.ParsedDocument.isPhrase(node, [106 /* ListIntrinsic */])) {
    ...
    _foreachStatement(node) {
        ...
        if (parsedDocument_1.ParsedDocument.isPhrase(value.expr, [154 /* SimpleVariable */])) {
            let varName = this._simpleVariable(value.expr);
            this.variableTable.setType(varName, type);
        }
        else if (parsedDocument_1.ParsedDocument.isPhrase(value.expr, [106 /* ListIntrinsic */])) {
