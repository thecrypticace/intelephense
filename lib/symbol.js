...
class ExpressionTypeResolver {
    constructor(nameResolver, symbolStore, lookupVariableTypeDelegate) {
        this.nameResolver = nameResolver;
    ...
    scopedMemberAccessExpression(node, kind) {
        ...
        let scopeTypeString;
        if (parsedDocument_1.ParsedDocument.isPhrase(node.scope, [83 /* FullyQualifiedName */, 139 /* QualifiedName */, 142 /* RelativeQualifiedName */])) {
            scopeTypeString = new TypeString(this.nameResolver.qualifiedNamePhraseText(node.scope, 1 /* Class */));
        }
        else if (parsedDocument_1.ParsedDocument.isPhrase(node.scope, [143 /* RelativeScope */])) {
    ...
    simpleVariable(node) {
        if (parsedDocument_1.ParsedDocument.isToken(node.name, [84 /* VariableName */])) {
            return this.lookupVariableTypeDelegate(this.nameResolver.tokenText(node.name), node.name.offset);
        }
        return null;
    }
