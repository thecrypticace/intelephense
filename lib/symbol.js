...
class SymbolTable {
    ...
    static create(parsedDocument) {
        let symbolReader = new SymbolReader(parsedDocument, new NameResolver(null, null, []), [{ kind: 0 /* None */, name: '', children: [] }]);
...
class SymbolReader {
    ...
    typeDeclaration(node) {
        return node.name.phraseType ?
            ...
            this.parsedDocument.tokenToString(node.name);
    }
    ...
    simpleVariable(node) {
        if (!parsedDocument_1.ParsedDocument.isToken(node, [84 /* VariableName */])) {
            return null;
        }
        return {
            kind: 256 /* Variable */,
...
class ExpressionResolver {
    ...
    simpleVariable(node) {
        ...
        return this.lookupVariableTypeDelegate(this.parsedDocument.tokenToString(node.name));
    }
    ...
    functionCallExpression(node) {
        ...
        let symbol = this.symbolStore.find(functionName, 64 /* Function */);
    ...
    methodCallExpression(node) {
        ...
        let methodName = parsedDocument_1.ParsedDocument.isToken(node.memberName) ?
            this.parsedDocument.tokenToString(node.memberName) :
