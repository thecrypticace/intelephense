...
class SymbolTable {
    ...
    get symbols() {
        ...
        let toArrayVisitor = new types_1.ToArrayVisitor();
    ...
    get count() {
        let traverser = new types_1.TreeTraverser([this.root]);
        let countVisitor = new types_1.CountVisitor();
    ...
    preOrder(node, spine) {
        ...
        switch (node.phraseType) {
            ...
            case 28 /* ClassDeclaration */:
                this._addSymbol(SymbolReader.classDeclaration(node, this.lastPhpDoc), true);
                return true;
            ...
            case 101 /* InterfaceDeclaration */:
                this._addSymbol(SymbolReader.interfaceDeclaration(node, this.lastPhpDoc), true);
                return true;
            ...
            case 163 /* TraitDeclaration */:
                this._addSymbol(SymbolReader.traitDeclaration(node, this.lastPhpDoc), true);
                return true;
    ...
    _token(t) {
        switch (t.tokenType) {
            case 160 /* DocumentComment */:
                this.lastPhpDoc = phpDoc_1.PhpDocParser.parse(SymbolReader.tokenText(t));
                break;
            case 119 /* CloseBrace */:
...
(function (SymbolReader) {
    ...
    function interfaceDeclaration(node, phpDoc) {
        let s = {
            kind: 2 /* Interface */,
            name: '',
            location: phraseLocation(node),
    ...
    function methodTagToSymbol(tag) {
        let s = {
            kind: 32 /* Method */,
            modifiers: 256 /* Magic */,
            name: tag.name,
    ...
    function magicMethodParameterToSymbol(p) {
        return {
            kind: 128 /* Parameter */,
            name: p.name,
            modifiers: 256 /* Magic */,
    ...
    function propertyTagToSymbol(t) {
        return {
            kind: 16 /* Property */,
            name: t.name,
    ...
    function traitDeclaration(node, phpDoc) {
        let s = {
            kind: 4 /* Trait */,
            name: '',
            location: phraseLocation(node),
    ...
    function classDeclaration(node, phpDoc) {
        let s = {
            kind: 1 /* Class */,
            name: '',
            location: phraseLocation(node),
