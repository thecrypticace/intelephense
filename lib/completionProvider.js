...
class NameCompletion {
    ...
    completions(context) {
        ...
        if (context.textBefore(3) === '<?p' &&
            parsedDocument_1.ParsedDocument.isToken(context.createTraverser().prevSibling(), [156 /* OpenTag */])) {
...
class ObjectAccessCompletion {
    ...
    canSuggest(context) {
        ...
        return parsedDocument_1.ParsedDocument.isToken(traverser.node, [83 /* Name */]) &&
            parsedDocument_1.ParsedDocument.isPhrase(traverser.parent(), [109 /* MemberName */]);
    }
    completions(context) {
        let traverser = context.createTraverser();
