...
function toClassCompletionItem(s, context, namePhraseType) {
    let item = {
...
function toFunctionCompletionItem(s, context, namePhraseType) {
    let item = {
    ...
    if (symbol_1.PhpSymbol.hasParameters(s)) {
        ...
        item.command = createSignatureHelpCommand(context.uri, lsp.Position.create(wordPos.line, wordPos.character));
...
function toConstantCompletionItem(s, context, namePhraseType) {
    let item = {
        kind: lsp.CompletionItemKind.Value,
        label: symbol_1.PhpSymbol.notFqn(s.name),
        documentation: s.description,
...
function toConstructorCompletionItem(s, context, namePhraseType) {
    let item = {
