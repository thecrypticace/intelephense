...
function toNameCompletionItem(s, namespace, namePhraseType, kind) {
    let label = s.name;
    let detail = s.name;
...
class ScopedAccessCompletion {
    ...
    completions(context) {
        let traverser = context.createTraverser();
        let accessee = traverser.ancestor(this._isScopedAccessExpr).scope;
