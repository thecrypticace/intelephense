...
class AbstractNameCompletion {
    ...
    _setInsertText(item, s, namespaceName, namePhraseType, useDeclarationHelper) {
        ...
        let notFqn = symbol_1.PhpSymbol.notFqn(namespaceName);
...
class SimpleVariableCompletion {
    ...
    _varTypeMap(s) {
        let map = {};
        let params = symbol_1.PhpSymbol.filterChildren(s, (x) => { return x.kind === 128 /* Parameter */; });
        for (let n = 0, l = params.length; n < l; ++n) {
            map[params[n].name] = symbol_1.PhpSymbol.type(params[n]);
        }
        let ref;
        if (!s.references) {
            return map;
...
class MemberAccessCompletion {
    ...
    completions(traverser, word) {
        ...
        let classAggregateType;
        if (classSymbol) {
            classAggregateType = new typeAggregate_1.TypeAggregate(this.symbolStore, classSymbol);
        }
        let typeName;
        let fn;
        let typeAggregate;
...
class ScopedAccessCompletion extends MemberAccessCompletion {
    ...
    _createMemberPredicate(scopeName, word, classContext) {
        if (scopeName === classContext.name.toLowerCase()) {
            ...
            return (x) => {
                return (x.modifiers & 32 /* Static */) > 0 && util.fuzzyStringMatch(word, x.name);
            };
        }
            ...
            return (x) => {
                return !(x.modifiers & 4 /* Private */) && util.fuzzyStringMatch(word, x.name);
            };
        }
...
class ObjectAccessCompletion extends MemberAccessCompletion {
    ...
    _createMemberPredicate(scopeName, word, classContext) {
        if (scopeName === classContext.name.toLowerCase()) {
            //public, protected, private
            return (x) => {
                return !(x.modifiers & 32 /* Static */) && util.fuzzyStringMatch(word, x.name);
            };
        }
        else if (classContext.isAssociated(scopeName)) {
            //public, protected
