...
class ClassBaseClauseCompletion extends AbstractNameCompletion {
    ...
    completions(context, maxItems) {
        if (!this._hasExtends(context)) {
            return { items: keywordCompletionItems(['extends'], context.word) };
        }
    ...
    _hasExtends(context) {
        return !!context.createTraverser().ancestor(this._isClassBaseClause).children.find((x) => {
            return x.tokenType === 30 /* Extends */;
        });
    }
...
class InterfaceClauseCompletion extends AbstractNameCompletion {
    ...
    completions(context, maxItems) {
        if (!this._hasKeyword(context)) {
            return { items: keywordCompletionItems(InterfaceClauseCompletion._keywords, context.word) };
        }
        return super.completions(context, maxItems);
    }
    _getKeywords() {
    ...
    _hasKeyword(context) {
        return !!context.createTraverser().ancestor(this._isInterfaceClause).children.find((x) => {
            return x.tokenType === 30 /* Extends */ || x.tokenType === 40 /* Implements */;
        });
    }
}
InterfaceClauseCompletion._keywords = ['extends', 'implements'];
