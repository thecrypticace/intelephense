...
CompletionProvider._defaultConfig = { maxItems: 100, enableAutoUseDeclarations: false };
...
class AbstractNameCompletion {
    ...
    completions(context) {
        ...
        let matches = this._mergeUseDeclarations(context.symbolStore.match(text, pred, true), context.nameResolver.rules.filter(pred));
    ...
    _toCompletionItem(s, context, namePhraseType) {
        ...
        if (this.config.enableAutoUseDeclarations &&
            ...
            namePhraseType === 140 /* QualifiedName */ &&
    ...
    _mergeUseDeclarations(matches, useDeclarations) {
        let merged = [];
        let map = {};
        let useDecl;
        let s;
