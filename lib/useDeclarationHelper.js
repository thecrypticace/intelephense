...
class UseDeclarationHelper {
    ...
    insertDeclarationTextEdit(symbol, alias) {
        ...
        let text = '\n';
        if (afterNode.phraseType === 120 /* NamespaceDefinition */) {
            text += '\n';
        }
        ...
        text += ';';
        if (afterNode.phraseType !== 124 /* NamespaceUseDeclaration */) {
            text += '\n';
        }
    ...
    findNamespaceUseClauseByRange(range) {
        let fn = (x) => {
            return (x.phraseType === 122 /* NamespaceUseClause */ || x.phraseType === 125 /* NamespaceUseGroupClause */) &&
                util.rangeEquality(range, this.doc.nodeRange(x));
        };
        return this.doc.find(fn);
    }
    _isUseDeclarationSymbol(s) {
        const mask = 1 /* Class */ | 64 /* Function */ | 8 /* Constant */;
        return (s.modifiers & 4096 /* Use */) > 0 && (s.kind & mask) > 0;
    }
    _insertAfterNode() {
        if (this._afterNode) {
    ...
    _isNamespaceAliasingClause(node) {
        return node.phraseType === 119 /* NamespaceAliasingClause */;
    }
}
...
class InsertAfterNodeVisitor {
    ...
    _isStatementList(node) {
        return node.phraseType === 157 /* StatementList */;
    }
}
