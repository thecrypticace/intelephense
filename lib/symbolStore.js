...
class ScopedVariablePruneVisitor {
    preorder(node, spine) {
        if ((node.kind === 64 /* Function */ || node.kind === 32 /* Method */) && node.children) {
...
class SymbolStore {
    constructor() {
        ...
        this._symbolIndex = new types_1.NameIndex(this._symbolKeys);
        this._symbolCount = 0;
    }
    getSymbolTable(uri) {
            ...
            case 16 /* Property */:
                {
                    let name = ref.name;
                    fn = (x) => {
                        return x.kind === 16 /* Property */ && name === x.name;
                    };
                    symbols = this.findMembers(ref.scope, memberMergeStrategy || 0 /* None */, fn);
                    break;
                }
    ...
    */
     ...
     */
    _indexFilter(s) {
        return !(s.kind & (128 /* Parameter */ | 4096 /* File */)) && //no params or files
            !(s.modifiers & (4096 /* Use */ | 4 /* Private */)) && //no use or private
            !(s.kind === 256 /* Variable */ && s.location) && //no variables that have a location (in built globals have no loc)
            s.name.length > 0;
    }
    _symbolKeys(s) {
        ...
        return symbol_1.PhpSymbol.keys(s);
    }
}
