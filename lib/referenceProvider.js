...
class ReferenceProvider {
    ...
    _variableReferences(symbol, refTable, symbolTable) {
        ...
        let isScope = (x) => {
            return x.kind === undefined && x.location && util.positionEquality(x.location.range.start, scopePositions[0]);
        };
    ...
    _symbolRefsInTableScope(symbol, refTable, filterFn) {
        ...
        if (traverser.find(findFn) && traverser.parent()) {
            let scope = traverser.node;
            return util.filter(scope.children, filterFn);
        }
        return [];
    }
}
