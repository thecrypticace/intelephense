...
class ReferenceStore {
    ...
    *knownDocuments() {
    ...
    close(uri) {
        ...
        if (table) {
            return this._cache.write(table.uri, table.root).catch((msg) => { logger_1.Log.warn(msg); });
        }
    ...
    fromJSON(data) {
        this._summaryIndex = new types_1.SortedList(ReferenceTableSummary.compare, data._summaryIndex);
        let items = this._summaryIndex.items;
        let item;
