...
(function (Intelephense) {
    ...
    const namespacedSymbolMask = 2 /* Interface */ |
        ...
        8 /* Constant */ |
        64 /* Function */;
    function openDocument(uri, documentText) {
        let doc = new document_1.TextDocument(uri, documentText);
        documentStore.add(doc);
        let parseTree = new parse_1.ParseTree(uri, php7parser_1.Parser.parse(documentText));
    ...
    function syncDocument(uri, documentText) {
        documentStore.remove(uri);
        parseTreeStore.remove(uri);
        symbolStore.remove(uri);
        openDocument(uri, documentText);
    }
    Intelephense.syncDocument = syncDocument;
