...
(function (Intelephense) {
    ...
    function initialise(options) {
        ...
        symbolCache = cache_1.createCache(path.join(storagePath, 'intelephense', 'symbols'));
        refCache = cache_1.createCache(path.join(storagePath, 'intelephense', 'references'));
        stateCache = cache_1.createCache(path.join(storagePath, 'intelephense', 'state'));
        ...
        if (options.clearCache) {
            ...
            return stateCache.read(stateTimestampKey).then((data) => {
                ...
                return cache_1.readArrayFromDisk(path.join(storagePath, 'intelephense', 'state', knownDocsFilename));
            }).then((uris) => {
    ...
    function shutdown() {
        ...
        return stateCache.write(stateTimestampKey, Date.now()).then(() => {
            return cache_1.writeArrayToDisk(uris, path.join(storagePath, 'intelephense', 'state', knownDocsFilename)).catch(() => { });
        }).then(() => {
    ...
    function cacheWriteReferenceStore() {
        ...
        if (data && data.length > 0) {
            return cache_1.writeArrayToDisk(data, path.join(storagePath, 'intelephense', refStoreTableSummariesFileName)).catch((e) => { });
        }
        else {
    ...
    function cacheReadReferenceStore() {
        let refStoreTables;
        return cache_1.readArrayFromDisk(refStoreTableSummariesFileName).then((items) => {
