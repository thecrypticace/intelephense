...
(function (Intelephense) {
    ...
    function initialise(options) {
        ...
        if (options.clearCache) {
            ...
            return stateCache.read(stateTimestampKey).then((data) => {
                ...
                return cache_1.readArrayFromDisk(path.join(storagePath, 'state', knowDocsFilename));
            }).then((uris) => {
    ...
    function shutdown() {
        ...
        return stateCache.write(stateTimestampKey, Date.now()).then(() => {
            return cache_1.writeArrayToDisk(uris, path.join(storagePath, 'state', knowDocsFilename)).catch(() => { });
        }).then(() => {
